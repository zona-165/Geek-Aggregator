...
export default function Home() {
  ...
  const [aiModelName, setAiModelName] = useState("deepseek-v4-flash");
  ...
  return <main className="shell">
    ...
    <section className="content">
      <header className="topbar"><div className="crumb">内容工作台 <span>/</span> {activeNav}</div><div className="top-actions"><a className="user-mini" href="/feed">查看前台 ↗</a><button className="icon-btn" onClick={() => flash("暂无新的系统通知")}>♧</button><button className="user-mini">林默 <span>⌄</span></button></div></header>
      ...
      {aiSettingsOpen && <section style={{marginBottom:24,padding:20,borderRadius:18,background:"#fff",border:"1px solid #e7eaf0"}}><div className="section-title"><div><h2>AI 接口配置</h2><span>暂不连接密钥，配置仅保存在当前页面</span></div><span className="status">未配置</span></div><div style={{display:"grid",gridTemplateColumns:"1.4fr 1fr 1.4fr",gap:12,marginTop:14}}><label>接口地址<input value={aiEndpoint} onChange={e => setAiEndpoint(e.target.value)} placeholder="https://api.example.com/v1" /></label><label>模型名称<input value={aiModelName} onChange={e => setAiModelName(e.target.value)} /></label><label>API Key（暂不保存）<input type="password" value={aiKey} onChange={e => setAiKey(e.target.value)} placeholder="后续由你填写" /></label></div><div style={{display:"flex",justifyContent:"flex-end",gap:10,marginTop:14}}><button className="filter" onClick={() => { setAiKey(""); flash("配置已清空"); }}>清空</button><button className="save-btn" onClick={() => { setAiKey(""); flash("配置已保留在当前页面，尚未写入服务器"); }}>保存配置</button></div></section>}
