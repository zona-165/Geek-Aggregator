...
function AdminHome() {
  ...
  const rewrite = async () => { if (!aiWritingEnabled) { flash("AI 写作已暂停，请先打开开关"); return; } if (!aiKey) { flash("请先在 AI 接口配置中填写 API Key"); setAiSettingsOpen(true); return; } flash("正在请求 DeepSeek 重写…"); try { const response = await fetch("/api/rewrite", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ endpoint: aiEndpoint, apiKey: aiKey, model: aiModelName || model, title: editTitle, content: selected.originalContent || editContent }) }); const data = await response.json(); if (!response.ok) throw new Error(data.error || "AI 请求失败"); const rewritten = data.content as string; const next = { ...selected, status: "已改写", rewrittenContent: rewritten, excerpt: rewritten }; setItems(xs => xs.map(x => x.id === selected.id ? next : x)); setSelected(next); setEditContent(rewritten); await fetch("/api/articles", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: selected.id, status: "已改写", rewrittenContent: rewritten }) }); setArticlePane("rewritten"); flash("DeepSeek 已生成重写内容"); } catch (error) { flash(error instanceof Error ? error.message : "AI 请求失败"); } };
